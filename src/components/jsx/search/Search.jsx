...
const Search = () => {
  ...
  return (
    <>
      <NavBar loginPopUp={false} />

      <div
        className="filter-display-box"
            ...
            >
              {isLoading ? (
                ...
                schools.map((schData) => {
                  // console.log(schData)
                  return (
                    <FilterschoolBox schData={schData} key={schData._id} />
                  );
                })
              )}
              {/* <FilterschoolBox /> */}
...
function FilterByBMR() {
  ...
  switch (filterValue) {
    case "Board":
      filterData = (
        <FilterByBoard filterData={fData[0]} title="FILTER BY BOARD" />
      );
      break;
    case "Medium":
      filterData = (
        <FilterByBoard filterData={fData[1]} title="FILTER BY Medium" />
      );
      break;
    case "Rating":
  ...
  return (
    <>
      <div className="filterBmrOuter">
        ...
        <div className="mb-tab-filterData">{filterData}</div>
      </div>
    </>
  );
}
...
function FilterByBoard(props) {
  ...
  async function fetchSchValue(fillValue) {
    ...
    try {
      ...
      const result = await axios.get(
        `${url}${filterEndPoint}?search=${encodeFilterValue}`
      );
  ...
  async function handleClick(val, con) {
    if (con) {
      ...
      filterSchList !== null ? dispatch(addSchool(filterSchList)) : null;
    } else {
      ...
      let fillValue = await filterValue
        .filter((fName) => fName !== val)
        .join(",");
      const filterSchList = await fetchSchValue(fillValue);
      filterSchList !== null ? dispatch(addSchool(filterSchList)) : null;
    }
